...
public:
	...
	//аналитическая зависимость dlyam/(du/dx)
	mytype d_lyam(mytype du_dx);
	mytype get_lyam_newton(mytype hx, mytype x, int p, vector<mytype>current, int i, int j);
	mytype get_dA_dq(mytype hx, int p, vector<mytype>current, int i, int j, int r);
	...
	void add_to_local_G_newton(int p, vector<mytype>current);
	void make_local_M(int p);
	void make_local_vec(int p, mytype t);
	void add_to_local_vec_newton(int p, mytype t, vector<mytype>current);
	void add_local_G(int p);
	void add_local_M(int p);
	void add_local_vec(int p);
	void make_global_G(vector<mytype> current);
	void make_global_G_newton(vector<mytype> current);
	void make_global_M(mytype k);
	void make_global_vec(mytype t);
	void make_global_vec_newton(mytype t, vector<mytype>current);
	void make_A();
	void make_d();
	void add_first(mytype t);
	//SLAU
	vector<mytype> next_vec(vector<mytype>& qc, vector<mytype>& qpr, mytype w);
	mytype R(mytype w);
	mytype Golden();
	void LU_sq();
...
//аналитическая зависимость dlyam/(du/dx)
template <typename mytype>
mytype SLAU<mytype>::d_lyam(mytype du_dx)
{
	return 2 * du_dx;
}

template <typename mytype>
mytype SLAU<mytype>::get_lyam_newton(mytype hx, mytype x, int p, vector<mytype>current, int i, int j)
{
	mytype first, second = 0;
	//arg=du/dx
	mytype arg = (current[p + 1] - current[p]) / hx;
	//первая		
	first = d_lyam(arg);
...
template <typename mytype>
mytype SLAU<mytype>::get_dA_dq(mytype hx, int p, vector<mytype>current, int i, int j, int r)
{
	mytype lyam1, lyam2;
	lyam1 = get_lyam_newton(hx, x[p], p, current, i, j);
	lyam2 = get_lyam_newton(hx, x[p + 1], p, current, i, j);

	if (i == r)
		return (lyam1 + lyam2) / (2 * hx);
	else
		return -(lyam1 + lyam2) / (2 * hx);
}
...
template <typename mytype>
void SLAU<mytype>::add_to_local_G_newton(int p, vector<mytype>current)
{
	mytype hx = x[p + 1] - x[p];

	mytype sum_q_r = current[p] + current[p + 1];
	...
	for (size_t i = 0; i < 2; i++)
	{
		for (size_t j = 0; j < 2; j++)
		{
			...
			for (size_t r = 0; r < 2; r++)
			{
				...
				sum_r += get_dA_dq(hx, p, current, i, j, r) * current[ql];
			}
			G[i][j] += sum_r;
		}
	}
...
template <typename mytype>
void SLAU<mytype>::add_to_local_vec_newton(int p, mytype t, vector<mytype>current)
{
	mytype f1, f2, hx;
	hx = x[p + 1] - x[p];
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			...
			for (size_t r = 0; r < 2; r++)
			{
				...
				sum_d_A += get_dA_dq(hx, p, current, i, r, j) * current[qr];
			}
...
template <typename mytype>
void SLAU<mytype>::make_global_G_newton(vector<mytype> current)
{
	for (int i = 0; i < m - 1; i++) {
		Gdi[i] = 0;
		Ggg[i] = 0;
	...
	{
		make_local_G(p, current);
		add_to_local_G_newton(p, current);
		add_local_G(p);
	}
}
...
template <typename mytype>
void SLAU<mytype>::make_global_vec_newton(mytype t, vector<mytype>current)
{
	for (int i = 0; i < m; i++)
		b[i] = 0;
	for (int p = 0; p < count_x - 1; p++)
	{
		make_local_vec(p, t);
		add_to_local_vec_newton(p, t, current);
		add_local_vec(p);
	}
}
...
//SLAE
template <typename mytype>
vector<mytype> SLAU<mytype>::next_vec(vector<mytype>& qc, vector<mytype>& qpr, mytype w)
{
	vector<mytype> new_q(qc.size());
	int size = qc.size();
	for (size_t i = 0; i < size; i++)
	{
		new_q[i] = w * qc[i] + (1 - w) * qpr[i];
	}
	return new_q;
}

template <typename mytype>
mytype SLAU<mytype>::R(mytype w)
{
	vector<mytype> new_q = next_vec(p_next, p_cur, w);
	make_global_G(new_q);
	make_global_vec(current_time);
	make_global_M(k);
	//make_global_vec_newton(times[cur_t], p_cur);
	make_A();
	make_d();
	add_first(current_time);
	mult(temp1, new_q, Agg, Agg, Adi);
...
template <typename mytype>
mytype SLAU<mytype>::Golden()
{
	mytype a = 0.1, b = 1.0, eps = 0.1;
	mytype r = (3 - sqrt(5)) / 2;
	mytype f1, f2;
	mytype x1 = a + r * (b - a);
	mytype x2 = b - r * (b - a);
...
void SLAU<mytype>::do_smth()
{
	...
	for (int cur_t = 1; cur_t < times.size(); cur_t++)
	{
		...
		mytype norm_nev = 0, norm_q = 0;

		for (int i = 0; i < m; i++)
		{
			norm_nev += (q_ist[i] - q_cur[i]) * (q_ist[i] - q_cur[i]);
		}
		norm_nev = sqrt(norm_nev);
		norm_q = norm(q_ist);
		norm_nev = norm_nev / norm_q;
		cout << "t=" << cur_t << "  " << norm_nev << endl;
	}
}

template <typename mytype>
void SLAU<mytype>::do_smth_newton()
{
	cout << scientific << setprecision(8);
